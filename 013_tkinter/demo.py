...
def adduser():
    ...
    email = t2.get()
    phone = t3.get()
    qry = "insert into Student1 values(%s,%s,%s,%s)"
...
def deleteuser():
    cursor.execute(f"delete from Student1 where id={id}")
    con.commit()
    for i in table.get_children():
        table.delete(i)
    t1.delete(0,END)
...
def updateuser():
    ...
    email = t2.get()
    phone = t3.get()
    qry = "update Student1 set name=%s,email=%s,phone=%s where id =%s"
...
b4 =Button(root,text="clear",width=15,command=clearuser).place(x=500,y=250)
